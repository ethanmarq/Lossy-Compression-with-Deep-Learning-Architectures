...
class SegmentationDataset(Dataset):
    ...
    def __init__(self, root_dirs, split='Train', transforms=None, compress='sz3', error='1E-1'):
    ...
    def _collect_image_mask_pairs(self):
        ...
        for root_dir in self.root_dirs:
            image_dir = os.path.join(root_dir, self.compress, self.error, self.split, 'rgb')
            mask_dir = os.path.join(root_dir, 'mixed', self.split, 'annos', 'int_maps')
            
            # Find images ending in .png and .jpg 
            images = sorted([os.path.join(image_dir, f) for f in os.listdir(image_dir) if f.endswith(('.png', '.jpg'))])
            ...
            # Adjust mask_dict to handle both .png and .jpg files
            mask_dict = {os.path.basename(mask).split('_')[1].replace(os.path.splitext(mask)[1], ''): mask for mask in masks}

            for img in images:
                key = os.path.basename(img).split('_')[1].replace(os.path.splitext(img)[1], '')
            ### - ### only does .png
