...
__all__ = ["parse_image_size", "random_drop_data", "DataProvider", "CityBaseDataProvider"]
...
class CityBaseDataProvider:
    ...
    def __init__(
        ...
        # Placeholders for additional params if needed
        #self.compress = None
        #self.error = None
        ...
        # build data loader
        self.train = self.build_dataloader(train_dataset, train_batch_size, n_worker, drop_last=False, train=True)
        self.valid = self.build_dataloader(val_dataset, test_batch_size, n_worker, drop_last=False, train=False)
        self.test = self.build_dataloader(test_dataset, test_batch_size, n_worker, drop_last=False, train=False)
        if self.valid is None:
            self.valid = self.test
