...
from efficientvit.segcore.data_provider import CityscapesDataProvider, RellisDataProvider, CavDataProvider
...
parser.add_argument("--compress", type=str, default='sz3', help="Compression type for dataset")
parser.add_argument("--error", type=str, default='1E-1', help="Compression Error Level for dataset")
...
def main():
    ...
    # Add compress and error into the configuration
    config['data_provider']['compress'] = args.compress
    config['data_provider']['error'] = args.error
    ...
    # setup 
    model = create_seg_model(config["net_config"]["name"], "cav", dropout=config["net_config"]["dropout"], weight_url=args.weight_url)
