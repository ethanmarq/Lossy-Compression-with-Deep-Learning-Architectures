...
class SegmentationDataset(Dataset):
    ...
    def _collect_image_mask_pairs(self):
        ...
        for img in images:
            key = os.path.basename(img).split('_')[1].replace('.jpg', '')
            if key in mask_dict:
    ...
    def train(self, num_epochs, save_interval=10, save_path='models'):
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            if epoch % save_interval == 0:
                directory_path = os.path.join(save_path, compressor, f'Q{quality_level}')
...
def plot_miou(trainer, num_epochs, save_path='results'):
    ...
    save_path = os.path.join(save_path, compressor, f'Q{quality_level}', f'miou_run{run}.svg')
    ...
    plt.xlabel('Epochs')
    plt.ylabel('MIoU')
    plt.title(f'UNet with ResNet Encoder Trained on Q{quality_level} Bounded Compressed Data')
    plt.legend()
    plt.grid(True)
