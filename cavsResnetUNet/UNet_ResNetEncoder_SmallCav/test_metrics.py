...
def pad_to_divisible_by_32(tensor):
    
    H, W = tensor.shape[-2:]
    
    pad_height = (32 - H % 32) % 32
    pad_width = (32 - W % 32) % 32
    return F.pad(tensor, (0, pad_width, 0, pad_height), mode='constant', value=0)
#####

def resize_to_divisible_by_32(image):
    W, H = image.size #reads in PIL Image
    
    new_H = (H + 31) // 32 * 32
    new_W = (W + 31) // 32 * 32
    
    print(f"new resize height is {new_H} and new resize width is {new_W}")
    
    return transforms.Resize((new_H, new_W), interpolation=Image.NEAREST) #pytorch expects (h,w) in that order
...
class Compose:
    ...
    def __call__(self, image, target):
        ...
        print(f"target shape before padding is {target.size}")
                
        target = resize_to_divisible_by_32(target)(target)
        image = resize_to_divisible_by_32(image)(image)
        
        print(f"target shape after padding is {target.size}")
        print(f"image shape after padding is{image.size}")
...
root_dirs = ['/scratch/aniemcz/CAT2/CAT/mixed', '/scratch/aniemcz/CAT2/CAT/Brown_Field', '/scratch/aniemcz/CAT2/CAT/Main_Trail', '/scratch/aniemcz/CAT2/CAT/Power_Line']
...
def get_inference_metrics(model_path, dataloader, batch_sizes, num_classes=4):
    ...
    print(f"MIoU: {sum(iou_distribution.values()) / len(iou_distribution)}")
...
if __name__ == "__main__":
    model_path = "models/sz3/1E-7/model_epoch_100_aug.pth"
