...
if len(sys.argv) > 1:
    ...
    compression_bound = sys.argv[2]
    #0 is base dataset no compression, 1 is 1E-1, 2 is 1E-2, etc up to 1E-7
    root_dir = "/scratch/aniemcz/cavsMiniLossyCompressorsV2/"
    train_imgs_dir = os.path.join(root_dir, f"{compressor}/1E-{compression_bound}/Train/rgb")
    train_masks_dir = os.path.join(root_dir, "mixed/Train/annos/int_maps")

    test_imgs_dir = os.path.join(root_dir, f"{compressor}/1E-{compression_bound}/Test/rgb")
    test_masks_dir = os.path.join(root_dir, "mixed/Test/annos/int_maps")
else:
    raise Exception("Need to pass the compressor name and compression bound as arguments")
# Custom Dataset
...
trainDS = SegmentationDataset(imgs_dir=train_imgs_dir, masks_dir=train_masks_dir, transforms=transform)
    ...
    def train(self, num_epochs, save_interval=10, save_path='models'):
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            if epoch % save_interval == 0:
                #model_save_path = os.path.join(save_path, compressor, f'1E-{compression_bound}', f'model_epoch_{epoch}_aug.pth')
                
                directory_path = os.path.join(save_path, compressor, f'1E-{compression_bound}')
                filename = get_unique_filename(save_path, f'model_epoch_{epoch}_aug_run{run}.pth')
                model_save_path = os.path.join(directory_path, filename)
...
def plot_miou(trainer, num_epochs, save_path='results'):
    epochs = np.arange(1, num_epochs + 1)
    miou_per_epoch = trainer.val_miou_per_epoch  # Use validation MIoU
    
    save_path = os.path.join(save_path, compressor, f'1E-{compression_bound}', f'miou_run{run}.svg')
    ...
    plt.xlabel('Epochs')
    plt.ylabel('MIoU')
    plt.title(f'UNet with ResNet Encoder Trained on {compressor} 1E-{compression_bound} run {run} Bounded Compressed Data')
    plt.legend()
    plt.grid(True)
