...
error_bound = ['Q0', 'Q20', 'Q40', 'Q60', 'Q80', 'Q100'] #called quality levels thats why its a Q with 0 being most lossy and lowest quality and 100 being highest quality but least lossy
...
for j, bound in tqdm(enumerate(error_bound)):
        ...
        for i in range(num_classes):
            df.loc[len(df)] = ['unet', compressor, error_bound[j], '120', mean_iou, classes[i], iou_per_class[i], 1]

df.to_csv(
    os.path.join(save_path, 'UNET_train_jpeg.csv')
)
